...
Invert the full PSM using the test samples.
...
system.time({
  realSD = jags.parallel(data.sd, NULL, parms, file.path(tempdir(), "fullFranks.txt"), 
                          n.chains = 4, n.iter = 2e6, n.burnin = 1e4, n.thin = 1e3)
})
```
...
system.time({
  stomata = jags.parallel(data.stomata, NULL, parms, file.path(tempdir(), "DonlyFranks.txt"), 
                          n.chains = 4, n.iter = 2e6, n.burnin = 1e4, n.thin = 1e3)
})
...
Without any carbon isotope data to constrain the gas exchange, and without constraints on the other morphology parameters, we're basically sampling all possible parameter space, and end up with little to no information in our posterior. The maximum in the pdf at ~500 ppm is interesting and I can dig into it but I'm pretty sure this just represents the most common `ca` solution space when sampling freely across all of our parameter distributions. It is probably also be somewhat influenced by the choice to center the weakly informative $\delta^{13}C_p$ distribution using a discrimination value of -19 permil, which I chose because it's a fairly typical 'modern' value. But the main point is that we've lost all distinction between samples here and end up with a very broad posterior.

## $\delta^{13}C_p$

Finally, let's look at a case where we only have a carbon isotope measurement for the plant (or perhaps dispersed organic carbon...) and no morphological information.

```{r d13C}
data.d13C = data.sd
data.d13C$d13Cp[5, 1] = -28.35
...
system.time({
  d13C = jags.parallel(data.d13C, NULL, parms, file.path(tempdir(), "d13ConlyFranks.txt"), 
                          n.chains = 4, n.iter = 2e6, n.burnin = 1e4, n.thin = 1e3)
})
